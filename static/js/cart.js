...
(() => {
    ...
    function showWompiInitError() {
        const message = 'No pudimos abrir la pasarela de pago. Intenta de nuevo o finaliza por WhatsApp.';
        if (typeof window.renderWompiFallback === 'function') {
            window.renderWompiFallback(message);
            return;
        }
    ...
    function ensureWompiReady() {
        if (typeof window.checkoutWompi === 'function' && window.checkoutWompi !== wompiCheckoutStub) {
            return Promise.resolve();
        }
        if (wompiLoader) return wompiLoader;
        wompiLoader = new Promise((resolve, reject) => {
            const script = document.createElement('script');
            const src = `${NB_BASE_PATH}static/js/wompi-estatico-completo.min.js`;
            script.src = src;
            script.defer = true;
